# Phase 7a — AWS RDS + Secrets Manager + Node Topology Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build (not deploy) the data-and-compute foundation of `eks-demo`'s real cloud-
production path — RDS Postgres with a Secrets Manager-backed credential flow, verified
Secrets Manager/External Secrets Operator wiring, and a two-node-group topology mirroring
kind's observability/apps separation with real multi-AZ HA — all `terraform validate`-clean
and ready to deploy, with zero live AWS deployment in this phase.

**Architecture:** Extends the existing (never-deployed) `infra/terraform/` +
...
(`ecr.tf`, `iam_irsa_external_dns.tf`, etc.).

**Tech Stack:** Terraform ~>1.5 (AWS provider ~>5.0, plus a new `random` provider),
`terraform-aws-modules/eks/aws` ~>20.0, External Secrets Operator `v1beta1`, Kustomize
overlays.

**Relationship to Phase 7b:** This plan was split from a single, larger Phase 7 plan (see
`docs/superpowers/specs/2026-07-14-phase7-aws-rds-secrets-manager-design.md`) because 13 tasks
across data/compute/observability/CDN/ingress was unwieldy for one review cycle. Phase 7a
covers spec sections 7.1, 7.2, 7.5, and 7.7 — the foundation Phase 7b's observability mirror,
Grafana ingress, and RDS-backed `postgres-exporter` all build on directly (Phase 7b's Task 1
onward assumes Phase 7a's node groups and RDS secret already exist). Section 7.3
(environment isolation) has no dedicated task in either plan — see this plan's own Final
whole-branch review section. Sections 7.4 (remote state), 7.6 (observability), 7.8 (static
CDN), and 7.9 (ingress/auth) all live in Phase 7b, not here — Phase 7a intentionally ends once
RDS + the two-node-group topology exist, without touching observability or the domain-facing
pieces at all.

**Domain (added after this plan's first draft):** the real domain for this deployment is a
delegated subdomain of `arwest.dev` — `openlex.arwest.dev` (already set as
`infra/terraform/terraform.tfvars.example`'s `domain_name` default). GitHub repo is
`rozdolsky33/OpenLex` (already `infra/terraform/variables.tf`'s planned `github_repository`
default in Phase 7b). Neither fact changes any task in this plan — `var.domain_name` isn't
consumed by RDS or node-group Terraform at all; it's here for context since both plans share
...
- **No static AWS credentials anywhere** — every AWS-authenticating workload uses IRSA
  (in-cluster), matching the existing `external-dns`/`external-secrets` precedent. Never add an
  `aws_iam_access_key`.
...
task, by design — no new code). 7.5 → folded into Task 1's `aws_db_instance` backup arguments.
7.7 → Tasks 4-5. (7.4, 7.6, 7.8, 7.9 live in Phase 7b — see this plan's "Relationship to Phase
7b" note.)

**Placeholder scan:** no TBD/TODO markers.

**Type/naming consistency:** `openlex.dev/workload` taint/label key and `observability`/`apps`
values are identical across Tasks 4-5 and kind's existing files (verified by Task 5 Step 5's
`grep`). `POSTGRES_EXPORTER_DSN`/`DATABASE_URL` key names in Task 1's `rds.tf` match what the
shared `infra/monitoring/postgres-exporter/values-base.yaml` (unchanged by this plan, already
correct) and Phase 7b's Task 4 (`postgres-exporter`) expect. `DATABASE_URL` scheme
(`postgresql+asyncpg://`) is consistent between Task 1's `rds.tf` and Task 2's `job.yaml`
(which explicitly strips `+asyncpg` before handing the URL to `psql`). The `gp3` `StorageClass`
Task 5 creates (default-class annotation) is what Phase 7b's Tasks 1-2 (Tempo, Loki) rely on
for their PVCs — confirmed by cross-reading both plans' text, not just asserted.

## Execution handoff to Phase 7b

Once this plan's tasks are complete and reviewed, `docs/superpowers/plans/
2026-07-14-phase7b-observability-cdn-ingress.md` picks up from here — its own Task 1
(observability's OTel Collector/Tempo/Jaeger) assumes this plan's Task 4-5 node groups and
Task 1's RDS secret already exist.
